...
export function defaultRecurrenceState(startsAt) {
...
function dateOnlyToFloatingUntil(dateStr) {
  return dateStr.replaceAll('-', '') + 'T235959Z';
}

export function buildRRuleString(state) {
...
export function parseRRuleString(rrule) {
  ...
  const state = { ...base, freq: fields.FREQ ?? '' };
...
export function describeRRule(state) {
