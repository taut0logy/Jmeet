...
type StartOptions = { audioDeviceId?: string; videoDeviceId?: string };
...
export function useLocalPreview({
...
}: { initialMicEnabled?: boolean; initialCameraEnabled?: boolean } = {}) {
  ...
  const start = useCallback(async ({ audioDeviceId, videoDeviceId }: StartOptions = {}) => {
    setError(null);
    stopStream();
    try {
